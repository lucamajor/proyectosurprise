...
const GameConfig = {
  ...
  obstacles: {
     ...
     * shrinks the hitbox on the left/right/top so near-misses feel fair. */
    variants: [
      { key: 'crate',  width: 40, height: 44, weight: 4, inset: 5,
        render: { src: null, emoji: null, color: '#ff0062' } },
      { key: 'tower',  width: 34, height: 62, weight: 3, inset: 5,
        render: { src: null, emoji: null, color: '#db2897' } },
      { key: 'wall',   width: 74, height: 38, weight: 2, inset: 5,
        render: { src: null, emoji: null, color: '#4b0433' } },
      { key: 'double', width: 40, height: 44, weight: 2, inset: 5, count: 2, spacing: 12,
        render: { src: null, emoji: null, color: '#ff0095' } },
    ],
  },
  ...
  theme: {
    ...
    groundImage: {
      // 128x34 table + chairs. The table top is 7 of those 34 rows down, which
      // is where offsetY comes from: 7/34 * height.
      src: 'assets/floor.png', mode: 'tile', speedRatio: 1,
...
class Obstacle extends Entity {
  ...
  constructor(game, variant, offsetX = 0) {
    ...
    const inset = (variant.inset || 0) * s;
    this.hitbox = { top: inset, right: inset, bottom: 0, left: inset };
  }
  ...
  draw(ctx) {
    ...
    ctx.beginPath();
    ctx.ellipse(this.x + this.width / 2, this.game.groundY + 2,
                this.width * 0.55, this.height * 0.07, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
...
class Game {
  ...
  spawnCollectibles() {
    ...
    for (let i = 0; i < group; i++) {
      // Spacing within a group is horizontal travel -> motion scale.
      const offsetX = i * cfg.groupSpacing * this.speedScale;
      // Arc the group upward in the middle so it traces a jump trajectory.
      const arc = group > 1 ? Math.sin((i / (group - 1)) * Math.PI) * 45 : 0;
      let h = (high ? cfg.highHeight : cfg.lowHeight) + arc;

      // Never bury a cupcake inside an obstacle: if one is close, float it above.
      const worldX = this.width + offsetX;
      const clash = this.obstacles.some(o =>
        Math.abs((o.x + o.width / 2) - worldX) < (o.width + cfg.width * this.scale));
      if (clash) h = Math.max(h, cfg.highHeight + 40);

      this.collectibles.push(new Collectible(this, offsetX, h));
    }
  }
...
function bindInput(game) {
  ...
  const press = (event) => {
    // Let the PLAY / RETRY button behave like a button.
    if (event.target && event.target.id === 'action-btn') return;
...
document.addEventListener('visibilitychange', () => {
  ...
  // Resuming from pause: any action button resumes rather than restarts.
  const originalAction = game.handleAction.bind(game);
  game.handleAction = () => {
    if (game.state === 'paused') {
      game.state = 'running';
      game.lastTime = performance.now();   // don't bank the paused seconds as dt
      game.ui.hideOverlay();
      return;
    }
    originalAction();
...
function pauseGame() {
  if (game.state === 'running') {
    game.state = 'paused';
    game.ui.overlay.classList.add('is-visible');
    game.ui.title.textContent = 'PAUSA';
    game.ui.text.textContent = '😴😴😴';
    game.ui.stats.hidden = true;
    game.ui.button.textContent = 'SEGUIR';
  }
}
